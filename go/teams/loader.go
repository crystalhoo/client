...
func (l *TeamLoader) load2InnerLockedRetry(ctx context.Context, arg load2ArgT) (*load2ResT, error) {
	...
	// xxx TODO record total calls and bytes in buckets
	// defer buckets.Log(ctx, "API.Calls")
	// defer buckets.Log(ctx, "API.Bytes")
	...
	if teamEnv.UserPreloadEnable {
		...
		if teamEnv.UserPreloadParallel {
			// xxx this is full-parallel. Probably want pipelining.
